...
file_path = Path('/Users/wesaman/cst205/pythonProject1/tmp0001.png')
if file_path.is_file():
    # If the file exists, read it
    image_data = imageio.imread(str(file_path))
else:
        ...
        for j in range(0, dims[1], 1):
            if 0 < old_array[i][j]:  # Check if a fish occupies the space
                if breed_time < old_array[i][j]:  # Check if the fish is elligible to breed
                    oldlocs = removeoccupied([[[i, (j + 1) % dims[1]], old_array[i][(j + 1) % dims[1]]],
                                              [[(i + 1) % dims[0], j], old_array[(i + 1) % dims[0]][j]],
                                              [[i, (j - 1) % dims[1]], old_array[i][(j - 1) % dims[1]]],
                                              [[(i - 1) % dims[0], j], old_array[(i - 1) % dims[0]][j]]])
                    newlocs = removeoccupied([[[i, (j + 1) % dims[1]], new_array[i][(j + 1) % dims[1]]],
                                              [[(i + 1) % dims[0], j], new_array[(i + 1) % dims[0]][j]],
                                              [[i, (j - 1) % dims[1]], new_array[i][(j - 1) % dims[1]]],
                                              [[(i - 1) % dims[0], j], new_array[(i - 1) % dims[0]][j]]])
                    availlocs = nestintersection(oldlocs,
                                                 newlocs)  # Determines which adjacent spaces are free in both arrays
                    ...
                    newlocs = removeoccupied([[[i, (j + 1) % dims[1]], new_array[i][(j + 1) % dims[1]]],
                                              ...
                                              [[(i - 1) % dims[0], j], new_array[(i - 1) % dims[0]][j]]])
                    availlocs = nestintersection(oldlocs,
                                                 newlocs)  # Determines which adjacent spaces are free in both arrays (i.e. actually empty)
                    if len(availlocs) != 0:  # Check if there are any options
                        chosenloc = availlocs[randint(0, len(availlocs) - 1)]  # Randomly choose one of the options
                        new_array[chosenloc[0]][chosenloc[1]] = old_array[i][
                                                                    j] + 1  # The fish moves to the new game array with an increment to its breeding timer
                    else:  # If there are no options, the fish does not move
                        new_array[i][j] = old_array[i][
                                              j] + 1  # The fish moves to the new game array with an increment to its breeding timer
                        old_array[i][
                            j] = 0  # Removes the fish from the previous game array (so that it doesn't move/breed twice)
                ...
                if -breed_energy > old_array[i][j]:  # Check if the shark is elligible to breed
                    ...
                    if len(availlocs) != 0:  # Check if there are any options
                        ...
                        new_array[chosenloc[0]][chosenloc[1]] = floor(
                            old_array[i][j] / 2)  # Add both sharks to the new array, sharing the energy between them
                        new_array[i][j] = floor(old_array[i][j] / 2)
                    else:  # If there are no options, the shark does not breed
                        new_array[i][j] = old_array[i][
                                              j] + 1  # The shark moves to the new game array with one less energy (or dies)
                else:  # Implement shark hunting
                    oldlocs = findfishoccupied([[[i, (j + 1) % dims[1]], old_array[i][(j + 1) % dims[1]]],
                                                [[(i + 1) % dims[0], j], old_array[(i + 1) % dims[0]][j]],
                                                [[i, (j - 1) % dims[1]], old_array[i][(j - 1) % dims[1]]],
                                                [[(i - 1) % dims[0], j], old_array[(i - 1) % dims[0]][j]]])
                    newlocs = findfishoccupied([[[i, (j + 1) % dims[1]], new_array[i][(j + 1) % dims[1]]],
                                                [[(i + 1) % dims[0], j], new_array[(i + 1) % dims[0]][j]],
                                                [[i, (j - 1) % dims[1]], new_array[i][(j - 1) % dims[1]]],
                                                [[(i - 1) % dims[0], j], new_array[(i - 1) % dims[0]][j]]])
                    availlocs = nestunion(oldlocs,
                                          newlocs)  # Find all nearby fish locations in either game array (whether they've been updated or not)
                    if len(availlocs) != 0:  # Check if there are any options
                        chosenloc = availlocs[
                            randint(0, len(availlocs) - 1)]  # Choose one of the adjacent fish randomly
                        if old_array[chosenloc[0]][
                            chosenloc[1]] > 0:  # Implement shark eating a fish that hasn't yet updated
                            new_array[chosenloc[0]][chosenloc[1]] = old_array[i][
                                                                        j] - energy_gain  # Moves the shark to the new game array with an increase in energy
                            old_array[chosenloc[0]][chosenloc[1]] = 0  # Removes the dead fish
                        else:  # Implement shark eating a fish that has already updated
                            new_array[chosenloc[0]][chosenloc[1]] = old_array[i][
                                                                        j] - energy_gain  # Moves the shark to the new game array with an increase in energy (automatically removes the dead fish)
                        ...
                        if len(availlocs) != 0:  # Check if there are any options
                            ...
                            new_array[i][j] = old_array[i][
                                                  j] + 1  # The shark moves to the new game array with one less energy (or dies)
                old_array[i][
                    j] = 0  # Removes the shark from the old game array (so that it doesn't eat/move/breed twice)
...
if basicSetup == True:  # The basic set-up with random placement of new fish and sharks
    ...
    for i in range(0, dims[0], 1):  # Cycle over all entries in the previous game array
        for j in range(0, dims[1], 1):
            if (i - dims[0] / 2) ** 2 + (
                    j - dims[1] / 2) ** 2 < initial_sharks / pi:  # Populate a central disk of sharks with random energy
                game_array[i][j] = randint(-breed_energy, -1)
            elif (i - dims[0] / 2) ** 2 + (j - dims[1] / 2) ** 2 < (
                    initial_sharks + initial_fish) / pi:  # Populate a ring of fish surrounding the sharks with random time
...
while k <= steps:
    ...
    if floor(k * 100 / steps) > prcnt:  # Output the progress of the simulation
        ...
        sys.stdout.flush()
        prcnt += 1
    if 0 < currcount[0] + currcount[1] < dims[0] * dims[
        1]:  # Check if the species have not gone extinct and the array isn't full
        k += 1
    else:
...
filename = 'SnFanimation_' + str(dims[1]) + 'x' + str(dims[0]) + '_(' + str(breed_time) + ',' + str(
    energy_gain) + ',' + str(breed_energy) + ')_(' + str(initial_sharks) + ',' + str(initial_fish) + ')_' + str(
    actual_steps) + '.gif'  # File name for the output gif file, includes relevant simulation parameters
...
images = []
image_paths = sorted(Path('.').glob('tmp*.png'))  # Ensure you're in the correct directory where images are saved
...
if images:
    ...
    imageio.mimwrite(gif_filename, images, fps=20)
    print(f"GIF created successfully at {os.path.abspath(gif_filename)}")
else:
    print("No images found or loaded for the GIF.")

# Optionally cleanup images after GIF creation if you don't want to keep them
for img_path in image_paths:
    os.remove(img_path)

# Move back to the initial directory if needed
os.chdir('..')
...
savefig('SnFplots_' + str(dims[1]) + 'x' + str(dims[0]) + '_(' + str(breed_time) + ',' + str(energy_gain) + ',' + str(
    breed_energy) + ')_(' + str(initial_sharks) + ',' + str(initial_fish) + ')_' + str(actual_steps) + '.png')
